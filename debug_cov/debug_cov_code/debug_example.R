...
# Setup reduced ################################################################
# library(DiceDesign)
# x <- lhsDesign(n = 20, dimension = 2, seed = 761)$design

# # space-filling design
# library(fields)
# x <- cover.design(R = expand.grid(seq(0, 1, length.out = 50), 
#                                   seq(0, 1, length.out = 50)), 
#                   nd = 20)
# x <- x$design

# symmetric design
x <- as.matrix(expand.grid(seq(0, 1, length.out = 5),
...
plot(x = x[,1], y = x[,2], pch = 16, xlab = "x1", ylab = "x2")

xstar <- rbind(c(3, 5), c(5, 3)) / 10
# xstar <- rbind(c(3, 5), c(7, 6)) / 10
points(x = xstar[,1], y = xstar[,2], pch = 8, col = "blue")
...
points(x = xprime1[,1], y = xprime1[,2], col = "red", pch = 15)
points(x = xprime2[,1], y = xprime2[,2], col = "red", pch = 15)
