...
N <- nrow(grid)
Kff <- covMatrix(X = grid, sig2 = 1, l = ll, covar.fun = r.matern) + diag( rep(1e-8, N) )
...
Kfy.joint <- covMatrix(X = rbind(grid, x), sig2 = 1, l = ll, covar.fun = r.matern)
Kyy <- covMatrix(X = x, sig2 = 1, l = ll, covar.fun = r.matern) + diag( rep(1e-8, length(y)) )
...
Kystary.full <- covMatrix(X = rbind(xstar, x), sig2 = 1, l = ll, covar.fun = r.matern)
Kystary <- Kystary.full[1:length(xstar), (length(xstar) + 1):ncol(Kystary.full)]
Kystarystar <- covMatrix(X = xstar, sig2 = 1, l = ll, covar.fun = r.matern)
